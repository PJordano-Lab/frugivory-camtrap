...
```{r }
Aune_data <- read.csv("/Users/Pablo/Documents/GitHub/donana-frugivory-camtrap/Code/data_example_raw_data/example_dataset.csv")
```

## 2. Include video duration                            - 10,827 rows
```{r }
#Summary list of file metadata extracted from Hard drive files
file_info <- read.csv ("/Users/Pablo/Documents/GitHub/donana-frugivory-camtrap/Code/data_example_raw_data/file_info.csv") %>%
...
```{r }
coord <- read.csv("/Users/Pablo/Documents/GitHub/donana-frugivory-camtrap/Code/data_example_raw_data/coordinates.csv") 
...
## 7. Remove unknown species and correct inconsitencies - 2,628 rows
```{r}
...
## 8. Collapse videos into 5 min events                 - 1,704 rows
```{r}
Aune_collapsed <- df1 %>%
...
## 9. Give final format to database for publication     - 1,704 rows
```{r }
