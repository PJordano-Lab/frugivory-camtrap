...
### 1. Read the data with sampling efforts 
```{r }
effort_yr1 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/sampling_effort/sampling_effort_yr1/Video_yr1.csv")
effort_yr2 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/sampling_effort/sampling_effort_yr2/video_yr2.csv", sep=";")
effort_cor_1 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/sampling_effort/sampling_effort_yr1/Corema_1_dates.csv", sep=",")  

effort_jpho <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Datos Jorge - Juniperus/sampling_effort_CT_JP.csv", sep= ",")
  
effort_plen <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/plen_adapted.csv")
```
...
#Pistacia lentiscus (sompling effort in Plen is refered to the ~ 2 hours that the GoPro are set divided by day, but only 12 hours because the study was focused only on birds)
...
## 1.2 COORDINATES
In this script we will give consistency and bind all the coordinates information for the 13 plant species selected for the datapaper. 
```{r }
#Sumhal coordinates - Include Corema coords (but not individual 25 -dont know why!?)
coord_sumhal_yr1 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/sampling_effort/Deployments_yr1.csv", sep=";") %>%
  mutate(Plant_ID = str_remove_all(Deployment_ID, "_"),
         Plant_ID = str_sub(Plant_ID, start = 1L, end = 7),
         Plant_ID = str_replace_all(Plant_ID, "A0", "calb0"),
         Plant_ID = str_replace_all(Plant_ID, "A1", "calb1"),
         Plant_ID = str_replace_all(Plant_ID, "A2", "calb2"),
         Plant_ID = str_to_lower(Plant_ID),
         Long = as.numeric(Long),
         Lat = as.numeric(Lat)) %>%
  select(Plant_ID, Long, Lat) %>%
  distinct()

#sumhal_coordinates yr2
coord_sumhal_yr2 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/sampling_effort/Deployments_yr2.csv", sep=";") %>%
  mutate(Plant_ID = str_remove_all(Deployment_ID, "_"),
         Plant_ID = str_sub(Plant_ID, start = 1L, end = 7),
         Plant_ID = str_to_lower(Plant_ID),
         Long = as.numeric(Long),
...
#Bind Sumhal yr1 and yr2 coordinates   
coord_sumhal <- rbind(coord_sumhal_yr1, coord_sumhal_yr2) %>%
  group_by(Plant_ID) %>%
...
#Corema coordinates
  coord_cor <- read.csv("/Users/Pablo/Documents/GitHub/SUMHAL_WP5_fieldwork/SUMHAL_fieldwork_Corema_1/corema_coords.csv", sep=",") %>%
  mutate(Plant_ID = str_replace_all(Name, "A","calb")) %>%
    select(Plant_ID, X, Y) %>%
    rename(Lat = Y, Long = X) %>%
    mutate(Lat = as.numeric(Lat),
           Long = as.numeric(Long)) %>%
    filter(Plant_ID == "calb25")

  
#Jpho coordinates
coord_jpho <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Datos Jorge - Juniperus/Juniper_coord.csv", sep =";") %>%
  select(tree,  y_long, x_lat) %>% #select only location and name from all individuals
  rename(Plant_ID = tree, Long = y_long, Lat = x_lat)%>%
  ...
  mutate(Plant_ID = str_replace_all(Plant_ID, "C", "jpho"),
         Plant_ID = str_replace_all(Plant_ID, "O", "jpho"),
         Plant_ID = str_replace_all(Plant_ID, "M", "jpho"),
         Long = as.numeric(Long),
         Lat = as.numeric(Lat))
...
#Plen coordinates
coord_plen <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Datos Elena - Pistacia/id_plant_coords_updated.csv",sep = ";") %>%
  rename(Plant_ID = id_plant, Long = X_coord, Lat = Y_coord) %>%
  mutate(Plant_ID = str_c("plen", Plant_ID, sep=""))
  

coordinates <- rbind(coord_sumhal, coord_cor, coord_jpho, coord_plen) 

#write.csv(coordinates, "/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/temp_results/coordinates/coordinates_13_spp.csv")
```
...
```{r}
d1 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Timelapse_results_Yr1/Phototrapping_data1.csv") %>%
  select(File, RelativePath, DateTime, Sp1, Behaviour, Count1, Sp2, Behaviour2, Count2, Sp3, Count3, Favourite, Obs)

d2 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Timelapse_results_Yr1/Phototrapping_data2.csv") %>% # (1) need to change "Pyrus_r6" to "Pyrus"
  select(File, RelativePath, DateTime, Sp1, Behaviour, Count1, Sp2, Behaviour2, Count2, Sp3, Count3, Favourite, Obs)

d3 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Timelapse_results_Yr1/Phototrapping_data3_Pyrus.csv") %>% # (2) need to add Pyrus to the file path
  select(File, RelativePath, DateTime, Sp1, Behaviour, Count1, Sp2, Behaviour2, Count2, Sp3, Count3, Favourite, Obs)

d4 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Timelapse_results_Yr1/Phototrapping_data4_MD_last_run.csv") %>% #Need to revise all the steps, because don´t know the triquiñuels
  select(File, RelativePath, DateTime, Sp1, Behaviour, Count1, Sp2, Behaviour2, Count2, Sp3, Count3, Favourite, Obs)

d5 <- read.csv("/Users/Pablo/Documents/GitHub/Animal-detection-cameratrap/Results/Timelapse_results_Yr1/Phototrapping_data5_joxy_yr1_revised_c05.csv") %>%
  select(File, RelativePath, DateTime, Sp1, Behaviour, Count1, Sp2, Behaviour2, Count2, Sp3, Count3, Favourite, Obs)
```
...
data_yr1 <- cbind(data_yr1,plant)

# NAME CORRECTIONS AND WILD ANIMAL SELECTION #
#Eliminate car, person, #equus cabalus, equus caballus, bos taurus
dat_yr1 <- data_yr1 %>%
  filter(Sp1 != "") %>%  #Only select videos with animals
  filter(Sp1 != "car") %>% #Eliminate car, person and empty videos
  filter(Sp1 != "person") %>%
  filter(Sp1 != "empty")
...
files_dat <- str_c(dat_yr1$RelativePath, dat_yr1$File, sep ="/")  #Lista de videos eating
files_dat_path <- file.path("/Volumes/G-RAID/SUMHAL", files_dat) #Lista de videos eating con su path
...
#Change Count to the number of individuals  
dat_yr1_duration$Count1 [dat_yr1_duration$Count1 == 0 ] <- 1
dat_yr1_duration$Count2 <- ifelse(dat_yr1_duration$Sp2 != "", 1, 0)
...
Plant_ID <- clean %>%
  ...
  mutate (Plant_ID = str_replace_all(Plant_ID, "a05\\\\100", "a05")) %>%   #Change the name for this corema that was wrong!! note the 4/ not 2
...
Rev <- data.frame (str_sub(clean$Rev, start = 1L, end = 5)) %>%
  ...
  mutate(Rev = str_replace_all(Rev, "rev", "rev0")) %>%  #Introduce 01, 02, 03 ... for consistencie when joining with sampling effort
...
#Non corrected TI
            Dt_correct = ifelse (ID == "rulm016_rev04_IMG_043", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "rulm021_rev03_IMG_091", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "rulm016_rev02_IMG_064", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "rulm016_rev02_IMG_070", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "rulm016_rev02_IMG_087", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "rulm016_rev02_IMG_092", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "pbou002_rev03_0306014", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "pbou002_rev03_0309054", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "pbou002_rev03_0309056", "xxx" , Dt_correct),
            Dt_correct = ifelse (ID == "pbou004_rev03_0113048", "xxx" , Dt_correct),
...
### 1. Load data                                      - 168,894 rows
   Load results for all species from visualisation in Timelapse and add a column with the plant species
```{r}
...
### 1. Load data 
indetermined videos reults from TimeLapse (YR1 & YR2)
```{r}
...
s_yr1 <- sumhal_yr1 %>%
  ...
  select(Sp1, Plant, Plant_ID, DateTime, Behaviour, Coexistence, n_cam, Days, Days.in.field, duration, ID, long, lat) %>%
...
cor_1 <- corema_1 %>%
  mutate(Plant = "calb",
        ...
        DateTime = as.POSIXct(DateTime)) %>%
  select(Sp1, Plant, Plant_ID, DateTime, Behaviour, Coexistence, n_cam, Days, Days.in.field, duration, ID, lat, long)
